...
const Comunicados = () => {
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-wrap items-end justify-between gap-4"><div><Badge className="mb-2 border-0 bg-primary/10 text-primary"><Megaphone className="mr-1 h-3 w-3" /> Boletim Conexão Feel</Badge><h1 className="font-display text-4xl font-bold">Comunicados oficiais</h1><p className="mt-1 text-muted-foreground">Canal real de comunicação interna entre setores da Lunks Feel.</p></div><Button variant="hero" size="lg" onClick={() => { setEditing(null); setOpen(true); }}><Plus className="h-4 w-4" /> Novo comunicado</Button></div>
